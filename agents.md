...
|---|---|---|
| `DRAG_THRESHOLD` | `6` px | Deslocamento mínimo desde o `pointerdown` para ativar o arrasto. Abaixo deste valor, o gesto é tratado como clique simples e abre o item normalmente. |
| `LONG_PRESS_DELAY` | `500` ms | Tempo de pressão contínua necessário para ativar o arrasto em dispositivos touch (`pointerType === 'touch'`). Abaixo deste tempo, o gesto é tratado como toque/scroll nativo. Em mouse, o arrasto ativa imediatamente (sem espera). |
...
**Check de complexidade ciclomática (validate.sh):** O `validate.sh` inclui
um check heurístico (Python) que analisa funções do JS inline e emite `[WARN]`
para funções com > 15 pontos de decisão (`if/else/for/while/switch/case/&&/||/?`).
Não bloqueia o pipeline (não incrementa `FAIL`). Funções com CC > 100 são
ignoradas (artefato do parser com template literals). As 12 funções que
atualmente disparam WARN são candidatas a refatoração futura — não são erros.
